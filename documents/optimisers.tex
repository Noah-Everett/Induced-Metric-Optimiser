...
The theoretical update $\delta\theta^i = -\eta\,l^i/(1+l_kl^k)$ is clean but not directly usable:
the denominator uses the exact gradient norm (unavailable with batched data),
there is no momentum, and there is no control over the strength of the metric correction.
We bridge this gap with four standard modifications, each motivated below.

\paragraph{Metric strength $\xi$.}
The theoretical derivation implicitly sets $\xi=1$.
In practice we introduce a tunable scalar $\xi>0$ that multiplies the rank-1 correction,
replacing $g_{ij} = \gamma_{ij} + l_il_j$ with $g_{ij} = \gamma_{ij} + \xi\,l_il_j$.
This controls how aggressively the induced metric damps the update:
$\xi\to 0$ recovers the base preconditioner $\gamma^{-1}$, while large $\xi$
makes the geometric clipping dominant.

\paragraph{Momentum.}
We maintain an exponential moving average (EMA) of the raw gradients,
$\vecb{m}_t = \beta_m\,\vecb{m}_{t-1} + (1-\beta_m)\,\vecb{l}_t$,
and step along the bias-corrected, metric-raised momentum $\hat{\vecb{m}}_t = \gamma^{-1}\vecb{m}_t/(1-\beta_m^t)$
rather than the instantaneous gradient.
This is the same momentum scheme used in Adam~\cite{harvey2025} and provides the
standard acceleration benefits.

\paragraph{Denominator EMA.}
The trace statistic $v_t = \xi\,\gamma^{mn}l_{t,m}l_{t,n}$ varies with each minibatch.
To stabilise the damping factor, we smooth $v_t$ with a second EMA:
...
Following AdamW, weight decay is applied directly to the parameters
($-\eta\lambda_\theta\theta_{t-1}$) rather than through the loss gradient.
As discussed in~\cite{harvey2025}, this decoupled form is the natural choice
from the geometric perspective due to general covariance.
...
\begin{figure}[ht]
    ...
    \caption{Step-size profile for the learnable scalar metric ($\xi=1$).
...
We adapt $s$ to keep $v_t$ in a useful operating range by performing
\emph{gradient ascent} on $v_t$ with respect to $s$:
$$
...
$$
whose derivative with respect to $s$ is $e^s/(1+\xi\,e^s\,l^2)^2 > 0$:
the step size is \emph{monotonically increasing} in $s$,
...
(i)~an L2 regularisation term $-\mu\lambda s$ that pulls $s$ back towards zero (i.e.\ $\gamma^{-1}=\Id$),
...
\begin{figure}[ht]
    ...
    \caption{Step-size profiles for off-diagonal variants in 1D ($\xi=1$).
    The one-sided and symmetric couplings provide progressively stronger damping
    compared to the fixed diagonal baseline (effective $\xi$ of $2$ and $3$ respectively).}
...
\STATE $\theta_t \leftarrow \theta_{t-1} - \eta\,(G^{-1}\vecb{r}) - \eta\,\lambda_\theta\,\theta_{t-1}$
\ENDWHILE
\RETURN $\theta_t$
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[ht]
    ...
    \caption{Step-size profiles compared across all variants ($\xi=1$).
    All induced-metric optimisers exhibit smooth gradient clipping relative to SGD.
    The learnable scalar ($s=1$) has a larger peak step size but clips at the same rate;
    the off-diagonal symmetric coupling clips more aggressively.}
