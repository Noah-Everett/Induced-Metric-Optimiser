...
\begin{itemize}
    \item \href{https://github.com/Noah-Everett/Induced-Metric-Optimiser/blob/main/analysis/small_examples_analysis.ipynb}{analysis/small\_examples\_analysis.ipynb} --- 2D synthetic functions (Beale, Rosenbrock, Himmelblau, Ackley, Rastrigin).
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.75\textwidth]{../images/step_profile_comparison.png}
    \caption{Step-size profiles compared across all variants
    in the single-parameter case ($N=1$, $\gamma=1$, $\xi=1$).
    All induced-metric optimisers exhibit smooth gradient clipping relative to SGD.
    The learnable scalar ($s=1$) has a larger peak step size;
    the off-diagonal symmetric coupling ($\vecb{a}=\vecb{b}=\vecb{l}$) clips more aggressively.}
