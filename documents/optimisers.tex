...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We now ask whether the induced metric $g_{ij}=\gamma_{ij}+\xi\,l_i\,l_j$
reduces the effective non-convexity of the loss function.
Concretely, we compute the \emph{Riemannian Hessian} of $\loss$ with respect to $g$
and investigate its relationship to the ordinary (Euclidean) Hessian
$H_{ij}\coloneqq\partial_i\partial_j\loss$.
All formulas in this section have been verified symbolically in Mathematica
for general $N$, and checked numerically on the benchmark functions
used in our experiments.

\subsection{Christoffel Symbols}\label{sec:christoffel}

The Christoffel symbols of the Levi-Civita connection
for the induced metric are
\begin{equation}\label{eq:christoffel}
    ...
    = \frac{1}{2}\,g^{kl}\bigl(
      ...
      \bigr).
\end{equation}
Since $\gamma$ is a constant (parameter-independent) diagonal metric,
the only $\theta$-dependence in $g_{ij}$ comes from the rank-one term
$\xi\,l_i\,l_j$, giving
\begin{equation}\label{eq:dg}
    \partial_k g_{ij}
    = \xi\bigl(H_{ki}\,l_j + l_i\,H_{kj}\bigr).
\end{equation}
Substituting into the Christoffel bracket and exploiting
the symmetry $H_{ij}=H_{ji}$, all cross-terms cancel:
\begin{equation}\label{eq:bracket}
    \partial_i g_{jl} + \partial_j g_{il} - \partial_l g_{ij}
    = 2\,\xi\,H_{ij}\,l_l.
\end{equation}
The contraction with the inverse metric yields a clean closed form:
using the Sherman--Morrison identity $g^{kl}\,l_l = l^k/(1+\xi\|l\|_\gamma^2)$, we obtain
\begin{equation}\label{eq:chris-closed}
    ...
    \;}
\end{equation}
where $l^k=\gamma^{kk}l_k$ is the metric-raised gradient
and $\|l\|_\gamma^2=\gamma^{mn}l_m l_n$.
In particular, $\Gamma^k_{ij}$ is symmetric in $i,j$ (as required)
and proportional to the Hessian $H_{ij}$.
When $\xi=0$ the expression vanishes, recovering the flat connection of $\gamma$.

\subsection{Riemannian Hessian}\label{sec:riem-hess}

The Riemannian Hessian (covariant Hessian) of $\loss$
with respect to $g$ is the $(0,2)$-tensor
\begin{equation}
    (\operatorname{Hess}_g\loss)_{ij}
    = H_{ij} - \Gamma^k_{ij}\,l_k.
\end{equation}
The correction term contracts the Christoffel symbol against the gradient:
\begin{equation}
    \Gamma^k_{ij}\,l_k
    = \frac{\xi\,H_{ij}\,l^k l_k}{1+\xi\,\|l\|_\gamma^2}
    = \frac{\xi\,\|l\|_\gamma^2}{1+\xi\,\|l\|_\gamma^2}\;H_{ij}.
\end{equation}
Therefore the Riemannian Hessian simplifies to a \emph{scalar rescaling}
of the Euclidean Hessian:
...
\begin{enumerate}[leftmargin=1.8em]
    \item \textbf{Eigenvalue scaling.}
    The eigenvalues of $\operatorname{Hess}_g\loss$ are exactly
    $\lambda_k(H)/(1+\xi\|l\|_\gamma^2)$, where $\lambda_k(H)$ are
    the eigenvalues of the Euclidean Hessian.
    The scaling factor $1/(1+\xi\|l\|_\gamma^2)\in(0,1]$ is strictly positive,
    so \emph{the sign of every eigenvalue is preserved}.

    \item \textbf{Geodesic convexity.}
    A function is geodesically convex with respect to $g$
    if and only if $\operatorname{Hess}_g\loss\succeq 0$.
    Since equation~\eqref{eq:riem-hess} preserves eigenvalue signs,
    $\loss$ is geodesically convex w.r.t.\ $g$
    \emph{if and only if} it is Euclidean-convex ($H\succeq 0$).
    The rank-one induced metric cannot convert a non-convex function
    into a geodesically convex one.

    \item \textbf{Condition number.}
    The condition number $\kappa(\operatorname{Hess}_g\loss) =
    \lambda_{\max}/\lambda_{\min}$ equals $\kappa(H)$:
    the uniform scaling cancels in the ratio.
    The metric does not improve or worsen the conditioning of the Hessian.

    \item \textbf{Curvature damping.}
    Although the metric does not change convexity, it \emph{reduces the
    magnitude of all curvatures} by a factor that grows with $\|l\|_\gamma^2$.
    Far from critical points (where the gradient is large),
    the effective curvature is strongly damped;
    near critical points ($\|l\|\to 0$), $\operatorname{Hess}_g\loss\to H$.
    This is the ``gradient clipping'' interpretation:
    the metric flattens the landscape in regions of steep gradient
    and leaves the local geometry near minima undistorted.
\end{enumerate}

\subsection{Connection to the Gauss Equation}\label{sec:gauss}

The embedding $\phi(\theta) = (\theta^1,\ldots,\theta^N,\loss(\theta))$
defines a hypersurface in $\mathbb{R}^{N+1}$ equipped with the ambient metric
$h=\operatorname{diag}(\gamma_1,\ldots,\gamma_N,\xi)$.
The intrinsic geometry of this hypersurface is governed by the
\emph{second fundamental form} $b_{ij}$ and the \emph{Gauss equation}.

\paragraph{Unit normal.}
The outward unit normal (with respect to $h$) is
\begin{equation}
    n = \frac{1}{\sqrt{\xi(1+\xi\|l\|_\gamma^2)}}
    \Bigl(-\xi\,l^1,\;\ldots,\;-\xi\,l^N,\;1\Bigr),
    \qquad
    n_3 \coloneqq \frac{1}{\sqrt{\xi(1+\xi\|l\|_\gamma^2)}}.
\end{equation}

\paragraph{Second fundamental form.}
Since $\partial_i\partial_j\phi = (0,\ldots,0,\,H_{ij})$,
...
\end{equation}
The second fundamental form is proportional to the Hessian,
and is related to the Riemannian Hessian by
\begin{equation}\label{eq:sff-hess}
    (\operatorname{Hess}_g\loss)_{ij} = n_3\,b_{ij}.
\end{equation}
This identity connects the covariant Hessian to the extrinsic curvature
of the loss surface: the Riemannian Hessian equals the second fundamental
form scaled by the normal component $n_3$.

\paragraph{Gauss equation and Riemann curvature.}
For a hypersurface in a flat ambient space, the Gauss equation gives
the intrinsic Riemann curvature tensor:
...
\end{equation}
This is the curvature tensor of $\det(H)$ restricted to
two-dimensional sections.
For a 2D parameter space, the Gaussian curvature is
\begin{equation}
    K = \frac{\xi\,\det(H)}{\det(\gamma)\,(1+\xi\|l\|_\gamma^2)^2},
\end{equation}
and the Ricci tensor satisfies $R_{ij} = K\,g_{ij}$, $R = 2K$.

\paragraph{Curvature--Hessian dictionary.}
Equation~\eqref{eq:gauss} provides a direct link between the intrinsic curvature
of the parameter surface and the Hessian of the loss:
\begin{itemize}[leftmargin=1.8em]
    \item Positive curvature ($K>0$) $\iff$ $\det(H)>0$ $\iff$ $H$ is definite
          (both eigenvalues have the same sign).
    \item Negative curvature ($K<0$) $\iff$ $\det(H)<0$ $\iff$ $H$ is indefinite
          (saddle point).
    \item Zero curvature ($K=0$) $\iff$ $\det(H)=0$ $\iff$ $H$ is singular.
\end{itemize}
The magnitude of the curvature is controlled by $\xi$:
larger $\xi$ increases $|K|$ when $\|l\|$ is small (near critical points)
and suppresses it when $\|l\|$ is large.

\subsection{Off-Diagonal Metric}\label{sec:geodesic-offdiag}

For the off-diagonal metric $g_{ij}=\gamma_{ij}+\xi(a_i l_j + b_j l_i + l_i l_j)$
with constant coupling vectors $a,b$,
...
\begin{enumerate}[leftmargin=1.8em]
    ...
    The bracket reduces to $2\xi(a_l+l_l)H_{ij}$, which is again proportional
    to $H_{ij}$.
    The Riemannian Hessian is therefore a (positive) scalar multiple of $H$,
    and the same sign-preservation result holds.
    However, the scalar factor now depends on $a$ and differs from
    the rank-one case.

    \item \textbf{Asymmetric case ($a\neq b$).}
    The extra term $(a_j-b_j)H_{il}$ breaks proportionality:
    the Riemannian Hessian correction becomes a non-trivial linear combination
    of Hessian slices in different directions.
    The resulting Hessian can have genuinely different eigenvalue structure
    from~$H$.
    However, the metric $g$ is not symmetric when $a\neq b$
    (unless $a-b\propto l$), so it does not define a Riemannian metric in the
    classical sense.
    The optimiser nonetheless uses this asymmetric $g$ as a preconditioner,
    and its effectiveness is an empirical question rather than a
    Riemannian-geometric one.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.8em]
    ...
    $\eta\,l^i$, allowing full steps near critical points where the gradient is informative.
\end{itemize}
...
illustrate these properties on the Rosenbrock and saddle functions.
