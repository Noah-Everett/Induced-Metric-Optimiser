...
$$
Here $\gamma_{ij}=e^{-s}\delta_{ij}$, so $l^i = \gamma^{ij}l_j = e^s\,\delta^{ij}l_j$
(identifying covectors and vectors via $\delta$, this is just a scale factor).
The parameter update is therefore
\begin{align*}
    \delta\theta^i
    &= -\eta\,g^{ij}l_j
    = -\eta\,l^i\left(1 - \frac{e^s\|\vecb{l}\|^2}{1+e^s\|\vecb{l}\|^2}\right)
    = -\frac{\eta\,l^i}{1 + e^s\|\vecb{l}\|^2}.
\end{align*}
...
\end{equation}
where $\vecb{m}$ is the bias-corrected momentum vector
(in practice, the update direction $\vecb{r}$ and the metric direction $\vecb{l}$
need not coincide; see Section~\ref{sec:shared} for the full treatment)
and $\hat{v}_t$ is the bias-corrected EMA of $v_t$.
For the log-loss embedding (Algorithm 2, using $f=\log\loss$ with $\loss>0$) the damping factor becomes
$r_t = \loss_t / (\loss_t^2 + |\hat{v}_t|)$.

\begin{tcolorbox}[colback=gray!8, colframe=gray!22, breakable, title=\textbf{Mapping to Paper (Alg.\ 1/2)}, coltitle=black]
    \begin{itemize}
        ...
        \item Update direction: $\hat{m}_t^i = \gamma^{ij} m_{t,j} = \alpha\,m_t^i$.
    \end{itemize}
    (We write $\tau_t$ for the paper's trace scalar to avoid collision with the log-scale $s$.)
\end{tcolorbox}
...
$$
Writing $l^i = \gamma^{ij}l_j = e^{s_i}\,\delta^{ij}l_j$, the update is
\begin{align*}
    \delta\theta^i
    &= -\eta\,g^{ij}l_j
    = -\eta\left(l^i - \frac{l^i\sum_k e^{s_k}l_k^2}{1+\sum_k e^{s_k}l_k^2}\right)
    = -\frac{\eta\,l^i}{1+\sum_k e^{s_k}l_k^2}.
\end{align*}
...
\begin{tcolorbox}[colback=gray!8, colframe=gray!22, breakable, title=\textbf{Mapping to Paper (Alg.\ 1/2)}, coltitle=black]
    \begin{itemize}
        ...
        \item Update direction: $\hat{m}_t^i = \gamma^{ij} m_{t,j} = e^{s_i}\,m_t^i$.
    \end{itemize}
    (We write $\tau_t$ for the paper's trace scalar to avoid collision with the log-scales $s_i$.)
\end{tcolorbox}
...
If the effective preconditioner is non-symmetric ($\vec{a}\neq\vec{b}$),
the update direction need not be a descent direction even for convex objectives.
We recommend defaulting to the symmetric case $\vec{a}=\vec{b}$;
if using $\vec{a}\neq\vec{b}$, consider symmetrising the effective preconditioner
...
for random $\vec{a},\vec{b},\gamma,\vecb{l}$ the closed-form $g^{ij}l_j$ matches
...
\begin{tcolorbox}[colback=gray!8, colframe=gray!22, breakable, title=\textbf{Quick Sanity Check}, coltitle=black]
    ...
    \medskip\noindent
    If we set $\vec{a}=\vec{0}$ but keep $\vec{b}\neq\vec{0}$,
    then $c_{al}=0$, $c_{ba}=0$, and $D = 1 + c_{bl} + c_{ll}$.
    The update reduces to
    $\delta\theta^i = -\eta\,l^i / D$,
    i.e.\ a scaled gradient step whose denominator depends on $\vec{b}$ through $c_{bl}$.
    This confirms that $\vec{b}$ affects the effective learning rate even when $\vec{a}=\vec{0}$.
\end{tcolorbox}
...
\begin{itemize}
    \item If $\vec{a}=\vec{0}$ or $\vec{a}\propto \vec{l}$, we have plain gradient descent with an adaptive learning rate.
